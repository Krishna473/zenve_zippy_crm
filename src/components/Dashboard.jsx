...
export default function Dashboard() {
      ...
      .then(
        ([
          ...
          const kpis = [
            ...
            { label: "PLATFORM COMMISSION", value: formatINR(platformCommission), sub: "From payouts" },
            { label: "STOCK VALUE", value: formatINR(stockValue), sub: `${inventory.length} inventory rows` },
            { label: "ORDERS", value: orders.length, sub: `${orders.filter((o) => o.payment_status === "paid").length} paid` },
            ...
            { label: "DOCTORS", value: doctors.length, sub: `${doctors.filter((x) => x.is_active === "Yes" || x.is_active === true).length} active` },
            { label: "SELLERS", value: sellers.length, sub: "Seller stores" },
            ...
            { label: "SALES EXECUTIVES", value: salesExecutives.length, sub: `${executiveTasks.length} tasks` },
  ...
  if (loading || !d) {
    return (
      <div className="zzc-content">
        <p className="zzc-muted">Loading dashboard…</p>
      </div>
    );
  }
    ...
    <div className="zzc-content">
      <div className="dash-hero">
        ...
        <p className="zzc-muted">
          Live view of the marketplace: veterinary network, pet commerce, services, pin-code coverage and the field sales engine —
          computed directly from your database.
        </p>
      ...
      <div className="dash-panel">
        ...
        <p className="zzc-muted dash-drill-note">
          Note: orders don't carry a direct store/pincode link in the database — they're matched to a store here by comparing each
          order item's product name against that store's inventory. If a backend update adds direct product/seller references to
          order items, this can switch to an exact join instead of a name match.
        </p>
      </div>
      ...
      <div className="dash-grid">
        ...
        <Panel
          ...
          subtitle="Doctors, SKUs and units per region"
          onExport={() =>
            downloadCSV(
              "pincode-depth.csv",
        ...
        <Panel
          ...
          subtitle="Open vs completed tasks (system-wide — executive_tasks has no per-executive field)"
